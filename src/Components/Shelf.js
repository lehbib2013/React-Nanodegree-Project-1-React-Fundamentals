...
const Shelf = ({shelfs,shelfDesignation,shelfBooks,onClassify})=>{
    ...
    return(
        ...
        {shelfBooks.length>0?shelfBooks.map((item) => {return (
        <Grid item xs={3} sm={4} md={4} key={item.book.id}>
       <Item key={item.book.id} >{<Book book={item.book} shelfs={shelfs} onClassify={onClassify} />}</Item>
      </Grid>)}):'No books on this shelf'}
       </Grid>
      </CardContent>
      </CustomizedCard>
